...
def special_Keys(commandString):
    ...
    if tempStr.join(commandString)[:9] == '<keySound':
        ...
        try:
            ...
            keySound = tempStr.join(commandString)[11:(len(commandString)-2)]

            handleMessages("Loaded sound file for keys", CNORMAL)

        except FileNotFoundError:

            handleError("Key sound file NOT find!")
    ...
    if tempStr.join(commandString)[:6] == '<sound':
        ...
        handleMessages('Playing sound file.', CNORMAL)
